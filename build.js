...
function build() {
    ...
    // ── random.js: original session-scoped behaviour (unchanged) ──────────────
    const jsContent = `
/**
...
`;
    ...
    console.log('Generated random.js (session mode)');

    // ── random_daily.js: one image per day per browser via localStorage ────────
    //
    // Only getRandomUrl() differs from random.js. All DOM logic below is identical.
    //
    // localStorage key format:  randompic_daily_{type}_{YYYY-MM-DD}
    //   e.g.  randompic_daily_h_2025-04-12
    //
    // Behaviour:
    //   • First visit of the day  → pick a random image, persist URL in localStorage.
    //   • Same day, any visit     → read stored URL and reuse it.
    //   • Next calendar day       → key no longer matches, fresh random is picked.
    //   • localStorage blocked    → silent fallback to per-session random (not persisted).
    const dailyJsContent = `
/**
 * Static Random Pic API
 * Generated by build script
 * Mode: daily — one image per day per browser, persisted via localStorage.
 */
(function() {
    var counts = ${JSON.stringify(counts)};
    var domain = '${config.domain}';
    ...
    // The stored image rotates at the user's local midnight.
    function todayStr() {
        var d = new Date();
    ...
    // Remove keys from previous days to keep localStorage tidy.
    function pruneOldKeys(type) {
        try {
            var today = todayStr();
            var prefix = 'randompic_daily_' + type + '_';
            var toDelete = [];
            for (var i = 0; i < localStorage.length; i++) {
                var k = localStorage.key(i);
                if (k && k.indexOf(prefix) === 0 && k.slice(prefix.length) !== today) {
                    toDelete.push(k);
                }
            }
    ...
    // In-memory fallback when localStorage is unavailable (e.g. private browsing).
    var sessionFallback = {};

    // Helper: Get random URL for a type (h or v), stable for the whole calendar day.
    function getRandomUrl(type) {
        if (!counts[type] || counts[type] === 0) return '';

        var key = 'randompic_daily_' + type + '_' + todayStr();

        // 1. Try reading today's stored URL
        try {
            var stored = localStorage.getItem(key);
            if (stored) return stored;
        } catch (e) {
            // localStorage unavailable — use in-memory fallback for this session
            if (!sessionFallback[type]) {
                var n = Math.floor(Math.random() * counts[type]) + 1;
                sessionFallback[type] = domain + '/ri/' + type + '/' + n + '.webp';
            }
            return sessionFallback[type];
        }

        // 2. Nothing stored yet for today — pick a fresh random image and save it
        var num = Math.floor(Math.random() * counts[type]) + 1;
        var url = domain + '/ri/' + type + '/' + num + '.webp';
        try {
...
`;
    fs.writeFileSync(path.join(DIST, 'random_daily.js'), dailyJsContent.trim());
    console.log('Generated random_daily.js (daily localStorage mode)');
